...
# real faults, aggregating the results for artificial faults and considering all scoring schemes.
# For each pair of techniques, the script performs a paired t test and
# computes the Cohen's d effect size.
#
# usage: Rscript replication_aggregated.R <data_file> <out_dir>
#
...
if (length(args)!=2) {
    stop("usage: Rscript replication_aggregated.R <data_file> <out_dir>")
}
data_file <- args[1]
out_dir <- args[2]
...
for (scheme in c("first", "median", "last")) {
    ...
    pdf(file=paste(out_dir, paste("distributions_aggregated_ratio_", initialCap(scheme), ".pdf", sep=""), sep="/"), pointsize=20, family="serif", width=30, height=7)
    ...
    pdf(file=paste(out_dir, paste("distributions_aggregated_ratio_per_project_", initialCap(scheme), ".pdf", sep=""), sep="/"), pointsize=20, family="serif", width=30, height=30)
    print(ggplot(df, aes(x=ScoreWRTLoadedClasses, color=FLT, linetype=Type)) + geom_line(stat="density", size=1.5) +
    scale_linetype_manual(name="     Family: ", values=c("twodash", "solid")) +
    theme_bw() +
    ...
    pdf(file=paste(out_dir, paste("distributions_aggregated_abs_", initialCap(scheme), ".pdf", sep=""), sep="/"), pointsize=20, family="serif", width=30, height=7)
    ...
    pdf(file=paste(out_dir, paste("distributions_aggregated_abs_per_project_", initialCap(scheme),".pdf",sep=""), sep="/"), pointsize=20, family="serif", width=30, height=30)
    print(ggplot(df, aes(x=ScoreAbs, color=FLT, linetype=Type)) + geom_line(stat="density", size=1.5) +
    scale_linetype_manual(name="     Family: ", values=c("twodash", "solid")) +
    theme_bw() +
    ...
    sink(paste(out_dir, paste("table_replication_aggregated_", initialCap(scheme), ".tex", sep=""), sep="/"))
