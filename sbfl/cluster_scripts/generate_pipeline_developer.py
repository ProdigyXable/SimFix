...
def do_pipeline_manual(project, bug, seed):
    global DATADIR
    global USERNAME

    script = "%s/pipeline-scripts/do-full-analysis" % os.environ['ANALYSIS_HOME']
    tmpdir="/scratch/%s/pipeline_%s_%s" % (USERNAME, project, bug)
    outdir="%s/scores/developer/%s/%s" % (DATADIR, project, bug)
    call = "if [ ! -e \"%s/scores.csv\" ]; then\n" % (outdir)
    call += "  if [ ! -e \"%s/mutation_files/developer/%s/%s/%s.killmap.gz\" ]; then\n" % (DATADIR, project, bug, bug)
    ...
    call += "  pushd . > /dev/null\n"
    call += "  cd %s\n" % tmpdir
    call += "  cp %s/mutation_files/developer/%s/%s/%s.killmap.gz .\n" % (DATADIR, project, bug, bug)
    call += "  cp %s/gzoltar_files/developer/%s/%s/matrix.gz .\n" % (DATADIR, project, bug)
    call += "  cp %s/gzoltar_files/developer/%s/%s/spectra.gz .\n" % (DATADIR, project, bug)
    call += "  cp %s/mutation_files/developer/%s/%s/mutants.log .\n" % (DATADIR, project, bug)
...
create_jobs(do_pipeline_manual, SCRIPTDIR, 0, 1)
