...
def do_pipeline_evosuite(project, bug, seed):
    global DATADIR
    global USERNAME

    script = "%s/pipeline-scripts/do-full-analysis" % os.environ['ANALYSIS_HOME']
    tmpdir="/scratch/%s/pipeline_evosuite_%s_%s" % (USERNAME, project, bug)
    outdir="%s/scores/evosuite/%s/%s" % (DATADIR, project, bug)
    call = "if [ ! -e \"%s/scores.csv\" ]; then\n" % (outdir)
    call += "  if [ ! -e \"%s/mutation_files/evosuite/%s/%s/%s.killmap.gz\" ]; then\n" % (DATADIR, project, bug, bug)
    ...
    call += "  pushd . > /dev/null\n"
    call += "  cd %s\n" % tmpdir
    call += "  cp %s/mutation_files/evosuite/%s/%s/%s.killmap.gz .\n" % (DATADIR, project, bug, bug)
    call += "  cp %s/gzoltar_files/evosuite/%s/%s/matrix.gz .\n" % (DATADIR, project, bug)
    call += "  cp %s/gzoltar_files/evosuite/%s/%s/spectra.gz .\n" % (DATADIR, project, bug)
    call += "  cp %s/mutation_files/evosuite/%s/%s/mutants.log .\n" % (DATADIR, project, bug)
...
create_jobs(do_pipeline_evosuite, SCRIPTDIR, 0, 1)
