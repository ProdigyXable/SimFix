...
TMPDIR=$(mktemp -d 2>/dev/null || mktemp -d -t 'killmaptmp')
CLASSPATH=$D4J_HOME/framework/lib/test_generation/generation/evosuite-current.jar:$file_jar $KILLMAP_HOME/scripts/generate-matrix.sh --relevant-test-classes $file_classes --triggering-tests $file_trigger $pid ${bid} $TMPDIR $OUT_DIR/${bid}.mutants.log 2> $OUT_DIR/${bid}.killmap-err.txt | gzip > $OUT_DIR/${bid}.killmap.gz
